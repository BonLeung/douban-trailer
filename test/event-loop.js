...
readFile('../package.json', 'utf-8', data => {
  ...
  readFile('../video.mp4', 'utf-8', data => {
    ...
    setImmediate(() => {console.log('[阶段3.immediate] 读文件回调2 增加的 immediate 回调7')})
  })
  ...
  setImmediate(() => {
    ...
    console.log('[....待切入下一阶段] 这块正在同步阻塞的读取一个大文件')
    ...
    readFile('../package.json', 'utf-8', data => {
      ...
      setImmediate(() => console.log('[阶段3 immediate] 读文件回调3 增加的 immediate 回调8'))
      setTimeout(() => console.log('[阶段1....定时器] 读文件回调3 增加的 定时器回调7'), 0)
    })
  })

  process.nextTick(() => console.log('[....待切入下一阶段] 读文件回调1 增加的 nextTick 回调6'))
